...
/// Google Workspace / Google Cloud Identity SSO provider
///
/// Supports authorization code flow with PKCE and device code flow.
/// Uses standard Google OAuth 2.0 / OpenID Connect endpoints.
/// Implements full JWT signature verification using Google's JWKS.
class GoogleProvider implements SSOProvider {
  ...
  static const _deviceCodeEndpoint = 'https://oauth2.googleapis.com/device/code';
  static const _jwksEndpoint = 'https://www.googleapis.com/oauth2/v3/certs';

  GoogleProvider(this.config) : _httpClient = HttpClient();

  @override
  String get providerId => 'google';

  @override
  String get displayName => 'Google Workspace';
  ...
  Future<Uri> getAuthorizationUrl({
    ...
    // If hosted domain is specified, restrict to that domain
    if (config.domain != null && config.domain!.isNotEmpty) {
      params['hd'] = config.domain!;
    }

    return Uri.parse(_authEndpoint).replace(queryParameters: params);
  }
  ...
  Future<TokenResponse> exchangeCode({
    ...
    final response = await _postForm(_tokenEndpoint, body);
  ...
  Future<IdTokenClaims> validateIdToken(String idToken) async {
    ...
    if (iss != 'https://accounts.google.com' && iss != 'accounts.google.com') {
      throw SSOException('Invalid issuer in ID token');
    }
    ...
    // Validate hosted domain if specified
    if (config.domain != null && config.domain!.isNotEmpty) {
      final hd = payload['hd'] as String?;
      if (hd != config.domain) {
        throw SSOException('User is not from the expected domain');
      }
    }
  ...
  /// Verify JWT signature using Google's JWKS
  ...
  Future<DeviceAuthResponse> startDeviceAuth() async {
    ...
    final response = await _postForm(_deviceCodeEndpoint, body);
